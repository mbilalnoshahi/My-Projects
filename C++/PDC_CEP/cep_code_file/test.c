...
int main ( );
unsigned char *julia_set
  ( int w, int h, float xl, float xr, float yb, float yt );
int julia
  ( int w, int h, float xl, float xr, float yb, float yt, int i, int j );
void tga_write
  ( int w, int h, unsigned char rgb[], char *filename );
...
{
  Timer t11;
  initTimer(&t11,"Total Time  ");
 // initTimer(&t22,"Time of 2nd Func (julia_set)");
  //initTimer(&t33,"Time of 3rd Func (tga_write)");
...
*/
  ...
  tga_write ( w, h, rgb, "julia_image.tga" );
  stopTimer(&t11);
/*
startTimer(&t1);
  Free memory.
...
#pragma omp parallel shared(h, w, xr, xl,  yt,yb ) private ( i, j, k, juliaValue ) 
    for ( j = 0; j < h; j++ )
    {
      #pragma omp  for
      for ( i = 0; i < w; i++ )
